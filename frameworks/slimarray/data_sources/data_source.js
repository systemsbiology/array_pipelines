    ...
    SC.Request.getUrl('/slimarray/projects').header({'Accept': 'application/json'}).json()
      ...
      if(parameters.scheme !== undefined) {
        if(parameters.scheme === null) {
          url += '&naming_scheme_id=nil';
        }
        else {
          url += '&naming_scheme_id=' + parameters.scheme.get('id');
        }
      }
      if(parameters.labGroup !== undefined) {
        url += '&lab_group_id=' + parameters.labGroup.get('id');
      }
      if(parameters.sampleNumber) {
        url += '&sample_number=' + parameters.sampleNumber;
      }
      ...
      if(parameters.scheme) {
        scheme_id = parameters.scheme.get('id');
      }
      else {
        scheme_id = 'nil';
      }
